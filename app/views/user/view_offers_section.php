...
<head>
    ...
    <?php include_once('css/baseOffer.php'); ?>
...
<body>
    <?php if ($_SESSION['role'] === 'pharmacy') { ?>
        <header style="padding-left: 20px">
            <h2 class='header'>Welcome to Seasonal Offers Section</h2>
    ...
    <div class='container'>
        <?php foreach ($this->results as $pharmId => $rows) { ?>
            <div class="container-fluid" id="<?= $pharmId ?>" style="cursor: pointer; border: 2px solid; border-radius: 10px; padding:30px;text-align:centre;">
                <span name='pharm-name'>
                    <h4><strong><?= $this->pharmacies[$pharmId][0]->name ?></strong></h4>
                </span>
                <span name='pharm-address'>
                    <h5><?= $this->pharmacies[$pharmId][0]->address ?></h5>
                </span>
